...
package org.dbiir.harp.utils.common.spi.type.typed;

import lombok.AccessLevel;
import lombok.NoArgsConstructor;
import org.dbiir.harp.utils.common.spi.exception.ServiceProviderNotFoundServerException;
...
public final class TypedSPILoader {
    ...
    public static <T extends TypedSPI> boolean contains(final Class<T> spiClass, final String type) {
        return HarpServiceLoader.getServiceInstances(spiClass).stream().anyMatch(each -> matchesType(type, each));
    }
    ...
    public static <T extends TypedSPI> Optional<T> findService(final Class<T> spiClass, final String type, final Properties props) {
        ...
        for (T each : HarpServiceLoader.getServiceInstances(spiClass)) {
    ...
    public static <T extends TypedSPI> boolean checkService(final Class<T> spiClass, final String type, final Properties props) {
        Collection<T> serviceInstances = HarpServiceLoader.getServiceInstances(spiClass);
