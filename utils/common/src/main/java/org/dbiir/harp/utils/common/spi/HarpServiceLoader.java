...
package org.dbiir.harp.utils.common.spi;
 ...
 */
public final class HarpServiceLoader<T> {
    
    private static final Map<Class<?>, HarpServiceLoader<?>> LOADERS = new ConcurrentHashMap<>();
    ...
    private HarpServiceLoader(final Class<T> serviceInterface) {
    ...
    public static <T> Collection<T> getServiceInstances(final Class<T> serviceInterface) {
        HarpServiceLoader<?> result = LOADERS.get(serviceInterface);
        return (Collection<T>) (null != result ? result.getServiceInstances() : LOADERS.computeIfAbsent(serviceInterface, HarpServiceLoader::new).getServiceInstances());
    }
