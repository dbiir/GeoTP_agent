...
public final class InternalMetaDataFactory {
     ...
     */
    public static AgentDatabase create(final String databaseName, final MetaDataPersistService persistService, final DatabaseConfiguration databaseConfig,
                                                final ConfigurationProperties props, final InstanceContext instanceContext) {
        return AgentDatabase.create(databaseName, DatabaseTypeEngine.getProtocolType(databaseName, databaseConfig, props), databaseConfig,
     ...
     * @return databases
     */
    public static Map<String, AgentDatabase> create(final MetaDataPersistService persistService, final Map<String, DatabaseConfiguration> databaseConfigMap,
                                                             final ConfigurationProperties props, final InstanceContext instanceContext) {
    ...
    private static Map<String, AgentDatabase> createDatabases(final MetaDataPersistService persistService, final Map<String, DatabaseConfiguration> databaseConfigMap,
                                                              final DatabaseType protocolType, final ConfigurationProperties props, final InstanceContext instanceContext) {
        Map<String, AgentDatabase> result = new ConcurrentHashMap<>(databaseConfigMap.size(), 1);
        for (Entry<String, DatabaseConfiguration> entry : databaseConfigMap.entrySet()) {
            ...
            if (!entry.getValue().getDataSources().isEmpty()) {
                ...
                result.put(databaseName.toLowerCase(), AgentDatabase.create(databaseName, protocolType));
            }
        }
        return result;
    }
