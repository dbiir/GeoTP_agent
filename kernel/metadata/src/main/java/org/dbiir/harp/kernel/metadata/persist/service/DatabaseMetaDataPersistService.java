...
package org.dbiir.harp.kernel.metadata.persist.service;

import lombok.Getter;
import org.dbiir.harp.kernel.metadata.persist.node.DatabaseMetaDataNode;
import org.dbiir.harp.kernel.metadata.persist.service.schema.TableMetaDataPersistService;
import org.dbiir.harp.kernel.metadata.persist.service.schema.ViewMetaDataPersistService;
...
public final class DatabaseMetaDataPersistService {
     ...
     */
    public void compareAndPersist(final String databaseName, final String schemaName, final AgentSchema schema) {
        if (schema.getTables().isEmpty() && schema.getViews().isEmpty()) {
            addSchema(databaseName, schemaName);
        }
        Map<String, AgentTable> currentTables = tableMetaDataPersistService.load(databaseName, schemaName);
     ...
     * @param schema schema meta data
     */
    public void persist(final String databaseName, final String schemaName, final AgentSchema schema) {
        if (schema.getTables().isEmpty() && schema.getViews().isEmpty()) {
            addSchema(databaseName, schemaName);
        }
     ...
     * @param schema schema meta data
     */
    public void delete(final String databaseName, final String schemaName, final AgentSchema schema) {
     ...
     */
    public Map<String, AgentSchema> loadSchemas(final String databaseName) {
        Collection<String> schemaNames = loadAllSchemaNames(databaseName);
        Map<String, AgentSchema> result = new LinkedHashMap<>(schemaNames.size(), 1);
        schemaNames.forEach(each -> result.put(each.toLowerCase(),
                new AgentSchema(tableMetaDataPersistService.load(databaseName, each), viewMetaDataPersistService.load(databaseName, each))));
        return result;
    }
