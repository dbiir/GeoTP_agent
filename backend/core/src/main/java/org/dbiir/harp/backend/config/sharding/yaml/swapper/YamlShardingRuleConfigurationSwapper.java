...
import org.dbiir.harp.backend.config.sharding.yaml.swapper.rule.YamlShardingTableReferenceRuleConfigurationConverter;
import org.dbiir.harp.backend.config.sharding.yaml.swapper.rule.YamlShardingTableRuleConfigurationSwapper;
...
//        data.getAutoTables().forEach(each -> result.getAutoTables().put(each.getLogicTable(), autoTableYamlSwapper.swapToYamlConfiguration(each)));
        result.getBindingTables().addAll(data.getBindingTableGroups().stream().map(YamlShardingTableReferenceRuleConfigurationConverter::convertToYamlString).toList());
