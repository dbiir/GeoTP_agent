...
package org.dbiir.harp.backend.config.sharding.rule;

import com.google.common.base.Splitter;
import com.google.common.base.Strings;
import lombok.Getter;
import org.dbiir.harp.backend.config.sharding.exception.metadata.ShardingTableRuleNotFoundException;
...
import org.dbiir.harp.utils.common.datanode.DataNode;
import org.dbiir.harp.utils.common.instance.InstanceContext;
import org.dbiir.harp.utils.common.instance.InstanceContextAware;
...
import org.dbiir.harp.utils.common.rule.identifier.scope.DatabaseRule;
...
//    private final KeyGenerateAlgorithm defaultKeyGenerateAlgorithm;
    ...
    public ShardingRule(final ShardingRuleConfiguration ruleConfig, final Collection<String> dataSourceNames, final InstanceContext instanceContext) {
        ...
        this.dataSourceNames = getDataSourceNames(ruleConfig.getTables(), dataSourceNames);
...
//        ruleConfig.getAuditors().forEach((key, value) -> auditors.put(key, TypedSPILoader.getService(ShardingAuditAlgorithm.class, value.getType(), value.getProps())));
        tableRules.putAll(createTableRules(ruleConfig.getTables()));
...
//        Preconditions.checkState(isValidBindingTableConfiguration(tableRules, new BindingTableCheckedConfiguration(this.dataSourceNames, shardingAlgorithms,
...
//        }
    ...
    private Collection<String> getDataSourceNames(final ShardingAutoTableRuleConfiguration shardingAutoTableRuleConfig) {
        List<String> actualDataSources = Collections.singletonList(shardingAutoTableRuleConfig.getActualDataSources());
    ...
    private Collection<String> getDataSourceNames(final ShardingTableRuleConfiguration shardingTableRuleConfig) {
        List<String> actualDataNodes = Collections.singletonList(shardingTableRuleConfig.getActualDataNodes());
        return actualDataNodes.stream().map(each -> new DataNode(each).getDataSourceName()).collect(Collectors.toList());
    }
    
    private Map<String, TableRule> createTableRules(final Collection<ShardingTableRuleConfiguration> tableRuleConfigs) {
...
//        return autoTableRuleConfigs.stream().map(each -> createAutoTableRule(each))
//                .collect(Collectors.toMap(each -> each.getLogicTable().toLowerCase(), Function.identity(), (oldValue, currentValue) -> oldValue, LinkedHashMap::new));
//    }
...
//        Preconditions.checkState(shardingAlgorithm instanceof ShardingAutoTableAlgorithm,
...
//    }
     ...
     */
    public boolean isAllBindingTables(final AgentDatabase database, final SQLStatementContext<?> sqlStatementContext, final Collection<String> logicTableNames) {
        ...
        AgentSchema schema = sqlStatementContext.getTablesContext().getSchemaName().map(database::getSchema).orElseGet(() -> database.getSchema(defaultSchemaName));
...
//        Optional<TableRule> tableRule = findTableRule(logicTableName);
//        Preconditions.checkState(tableRule.isPresent(), () -> new GenerateKeyStrategyNotFoundException(logicTableName));
...
//    private boolean isJoinConditionContainsShardingColumns(final AgentSchema schema, final SelectStatementContext select,
...
//    }
//
//    private Collection<String> getJoinConditionTables(final AgentSchema schema, final SelectStatementContext select,
